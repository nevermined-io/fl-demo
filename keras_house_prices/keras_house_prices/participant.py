...
class Participant(  # pylint: disable=too-few-public-methods,too-many-instance-attributes
    ...
    def load_dataset(self):
        for file_path in Path(INPUTS_PATH).rglob("*"):
            if file_path.is_file() and file_path.name.startswith("data_part"):
                LOG.info(f"Train on dataset: {file_path.as_posix()}")
                trainset = pd.read_csv(file_path.as_posix(), index_col=None)
                break
        ...
        test_file_path = Path("/xain-participant/test.csv")
        LOG.info(f"Test dataset: {test_file_path.as_posix()}")
        testset = pd.read_csv(test_file_path.as_posix(), index_col=None)
